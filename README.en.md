...
npm run test-firefox        # the same 1159 assertions, under Gecko
```
...
> **ONE OF THESE ASSERTIONS HAD TO BE REDONE, and the mistake is instructive.**
> The first required the bubble to pass `#side-nav`'s right edge — but the test
> fixture has no Twitch stylesheet: its bar takes the WHOLE page, so a 330 px
> card passed nothing at all. Measured: `depasse: -934`. It was not measuring
> the product, it was measuring the fixture. The scenario now sets the real
> width — two hundred and forty pixels — for the duration of the measurement.
...
| `npm test` | the Playwright harness: 127 scenarios, 1159 assertions |
...
| --- | --- | --- | --- |
| `content.js` | 1057 KB | 404 KB | 3,323 → **2** |
